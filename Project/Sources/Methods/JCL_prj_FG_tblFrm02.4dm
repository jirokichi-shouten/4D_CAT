...
//フォームのタイトル文字列
$title_body:=JCL_prj_FG_frm02_title($folderPath; $objParam)
$body:=Replace string:C233($body; "[--frm_varTitle]"; $title_body)

//フォームのOKボタン
$btn_body:=JCL_prj_FG_frm_obj($folderPath; $objParam.frm_prefix; "frm_btnOK.txt")  //template file name
$body:=Replace string:C233($body; "[--frm_btnOK]"; $btn_body)  //form_template tag
//オブジェクトメソッドを作成
JCL_prj_FG_tblObjMethod($objParam; "_btnOK")  //method generate

//フォームのCancelボタン
$btn_body:=JCL_prj_FG_frm_obj($folderPath; $objParam.frm_prefix; "frm_btnCancel.txt")
$body:=Replace string:C233($body; "[--frm_btnCancel]"; $btn_body)
//オブジェクトメソッドを作成
JCL_prj_FG_tblObjMethod($objParam; "_btnCancel")

//フォームのDeleteボタン
$btn_body:=JCL_prj_FG_frm_obj($folderPath; $objParam.frm_prefix; "frm_btnDelete.txt")
$body:=Replace string:C233($body; "[--frm_btnDelete]"; $btn_body)
//オブジェクトメソッドを作成
JCL_prj_FG_tblObjMethod($objParam; "_btnDelete")

//フィールド用の文字列を作成、複数フィールドを作成
$lst_body:=JCL_prj_FG_frm_flds($folderPath; $objParam.frm_prefix; $aryFieldNamePtr; $aryFieldTypePtr; $aryFieldLengthPtr)
...
$outFolderPath:=JCL_file_MakeFilePath($folderPath; $objParam.frm_name)
