...
JCL_pgs_DefInit
JCL_pgs_Show("郵便番号(事業所）を読み込んでいます．．．")

//すでにある県の郵便番号レコードを削除
READ WRITE:C146([Z_PostalCode:8])
QUERY:C277([Z_PostalCode:8]; [Z_PostalCode:8]PO_JIGYOSYO:6=1)
...
For ($i; 2; $numOfLines)  //2行目から読む
	If (Mod:C98($i; 1000)=0)
		JCL_pgs_SetValue(($i/$numOfLines)*100; "事業所："+$address; $i; $numOfLines)
	...
	If ($minimumCols<=$numOfItems)
		//DBに保存
		PO_Add_byInitValues
		[Z_PostalCode:8]PO_CODE:2:=Replace string:C233($itemAry{8}; "\""; "")  //郵便番号（7桁）
		[Z_PostalCode:8]PO_PREFECTURE:3:=Replace string:C233($itemAry{4}; "\""; "")  //都道府県名
		[Z_PostalCode:8]PO_CITY:4:=Replace string:C233($itemAry{5}; "\""; "")  //市区町村名
		[Z_PostalCode:8]PO_TOWN:5:=Replace string:C233($itemAry{6}+$itemAry{7}; "\""; "")  //町域名
		[Z_PostalCode:8]PO_JIGYOSYO:6:=1  //事業所
		SAVE RECORD:C53([Z_PostalCode:8])  //レコードを保存
		
		$address:=$itemAry{4}+$itemAry{5}+$itemAry{6}+$itemAry{7}
	End if 
