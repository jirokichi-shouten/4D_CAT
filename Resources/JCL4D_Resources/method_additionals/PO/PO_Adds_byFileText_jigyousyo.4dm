...
QUERY([Z_PostalCode]; [Z_PostalCode]PO_JIGYOSYO=1)
...
For ($i; 2; $numOfLines)  //2行目から読む
	...
	If ($minimumCols<=$numOfItems)
		//DBに保存
		PO_Add_byInitValues
		[Z_PostalCode]PO_CODE:=Replace string($itemAry{8}; "\""; "")  //郵便番号（7桁）
		[Z_PostalCode]PO_PREFECTURE:=Replace string($itemAry{4}; "\""; "")  //都道府県名
		[Z_PostalCode]PO_CITY:=Replace string($itemAry{5}; "\""; "")  //市区町村名
		[Z_PostalCode]PO_TOWN:=Replace string($itemAry{6}+$itemAry{7}; "\""; "")  //町域名
		[Z_PostalCode]PO_JIGYOSYO:=1  //事業所
